// DS TM - Mexikaner Edition (DS-Tools module)

(function () {
  "use strict";

  const params = new URLSearchParams(location.search);
  if (params.get("screen") !== "place" || params.get("try") !== "confirm") {
    return;
  }

  if (window.__DS_TIMING_MASTER_RUNNING__) return;
  window.__DS_TIMING_MASTER_RUNNING__ = true;
  ...
  if (!confirmationForm || document.getElementById(ID.incomingsDiv)) return;
  confirmationForm.insertAdjacentHTML("afterend", mainUI);

  const relativeTimeElem = document.querySelector(".relative_time");
  if (relativeTimeElem) {
    document.getElementById(ID.arrivalTimeInput).value =
      relativeTimeElem.textContent.slice(-8);
  }
  ...
  function freezeUI() {
    ...
    if (container) {
      container.innerHTML =
        '<div style="padding:10px; background:#dec6aa; text-align:center; font-weight:bold; font-size:14px; border:1px solid #7d510f;">Mexikaner regelt</div>';
    }
  }
      ...
      $.get(villageUrl).done((html) => {
        ...
        if (commandsTable.length > 0) {
          ...
          commandsTable.find("tr.command-row").each(function () {
            const isReturning =
              $(this).find('img[src*="/return_"], img[src*="/back.png"]').length > 0;
            if (isReturning) {
              $(this).remove();
              return;
            }
            ...
            $(this).append(
              `<td class="timingMasterSendTime"><b><span class="timer" style="color: darkblue" data-endtime="${sendTimeMs / 1000}"></span></b></td>`
            );
            ...
            $(this).on("click", function () {
              ...
              const timeString = arrivalDate.toLocaleTimeString("de-DE", {
                hour12: false,
              });
